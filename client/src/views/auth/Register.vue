  ...
  <div class="flex justify-center py-8">
    <div class="w-[640px]">
      ...
      <h2 class="text-2xl font-semibold mb-4">
        {{ form.state === FormState.USER_DATA ? 'Enter Your User Data' : 
            form.state === FormState.VERIFY ? 'Verify Email Address' : 'Register New Account' }}
      </h2>
      ...
      <form>
        ...
        <div v-show="form.state === FormState.EMAIL">
          ...
          <div class="mb-4">
            ...
            <input v-model.trim="form.data.email" type="email" id="email" name="email" 
        ...
        <div v-show="form.state === FormState.USER_DATA">
          ...
          <div class="mb-4">
            <label for="firstname" class="block text-gray-600 text-sm font-medium mb-2">Firstname</label>
            <input v-model.trim="form.data.firstname" type="text" id="firstname" name="firstname" 
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500" required>
          </div>

          <div class="mb-8">
            <label for="lastname" class="block text-gray-600 text-sm font-medium mb-2">Lastname</label>
            <input v-model.trim="form.data.lastname" type="text" id="lastname" name="lastname" 
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500" required>
          </div>
          ...
          <div class="mb-4">
            ...
            <input v-model="form.data.password" type="password" id="password" name="password" 
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500" required>
          </div>

          <div class="mb-4">
            <label for="confirm" class="block text-gray-600 text-sm font-medium mb-2">Confirm password</label>
            <input v-model="form.data.confirm" type="password" id="confirm" name="confirm" 
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500" required>
          </div>
        </div>
        ...
        <div v-show="form.state === FormState.VERIFY">
          ...
          <div class="mb-4">
            <label for="verificationCode" class="block text-gray-600 text-sm font-medium mb-2">Verification Code</label>
            <input v-model.trim="form.data.verificationCode" type="text" id="verificationCode" name="verificationCode" 
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500" required>
          </div>

          <p class="mb-4">
            Wrong email address? <a href="/register" target="_blank" class="text-blue-500 hover:text-blue-600">Start a new registration process</a>
          </p>
        </div>
        ...
        <div v-show="form.state === FormState.EMAIL" class="text-center">
          <RouterLink to="/login" class="text-blue-500 hover:text-blue-600">Already have an account? Login</RouterLink>
        </div>
      </form>
    </div>
