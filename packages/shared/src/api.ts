...
export const commandRequestSchema = z.discriminatedUnion('type', [
  ...
  request('SetTaskParent', setTaskParentParams),
...
export const taskNodeSchema = z.object({
  ...
  /** ISO 1–7, sorted. The other half of "Tuesdays in the afternoon" (§6.5). */
  ownPreferredWeekdays: z.array(z.int().min(1).max(7)).nullable(),
  effectivePreferredWeekdays: z.array(z.int().min(1).max(7)).nullable(),
